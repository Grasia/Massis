...
/**
 * Implements three algorithms that clean up issues that can develop around null
 * region boarders.
 * 
 * <p>
...
public class CleanNullRegionBorders
	 ...
	 * Design Notes:
	 * 
	 * Three algorithms have been aggregated into this single class for
	 * performance reasons. Otherwise we would be stuck performing three full
	 * contour searches rather than one.
	 * 
	 * The optimization method used in the search can result in missed null
	 * region contours. Consider the following pattern:
	 * 
	 * x x x x x x x a a a a x x - null region WITHOUT SPANS x a v v a x a -
	 * region A x a v v a x v - null region WITHOUT SPANS x a a a a x x x x x x
	 * x
	 * 
	 * If an all span search is performed, the outer null region (x) will be
	 * detected, but during the process all a-region spans will be marked as
	 * viewed. This will leave no spans available to detect the inner null
	 * region (v).
	 * 
	 * I've not fixing this until it proves to be a problem or I figure out a
	 * way of resolving the design issue without killing performance.
	 */
	...
	private final ArrayDeque<OpenHeightSpan> mwOpenSpans = new ArrayDeque<OpenHeightSpan>(
			1024);
	private final ArrayDeque<Integer> mwBorderDistance = new ArrayDeque<Integer>(
			1024);
	 ...
	 *            If TRUE, then only null region spans will be used to initially
	 *            detect null region borders. This improves performance. If
	 *            FALSE, all spans are searched to detect borders.
	 */
	...
	public void apply(final OpenHeightfield field)
	{
		...
		while (iter.hasNext())
		{
			...
			final boolean isEncompassedNullRegion = processNullRegion(
					workingSpan, edgeDirection);
	 ...
	 *            The hard border for flooding. No spans in this direction from
	 *            the startSpan will be flooded.
	 * @param newRegionID
	 *            The region id to assign the flooded spans to.
	 */
	private void partialFloodRegion(final OpenHeightSpan startSpan,
			final int borderDirection, final int newRegionID)
	{
	...
	/**
	 * Detects and fixes bad span configurations in the vicinity of a null
	 * region contour. (See class description for details.)
	 * 
	 * @param startSpan
	 *            A span in a non-null region that borders a null region.
	 * @param startDirection
	 *            The direction of the null region border.
	 * @return TRUE if the start span's region completely encompasses the null
	 *         region.
	 */
	private boolean processNullRegion(final OpenHeightSpan startSpan,
			final int startDirection)
	{

		/*
		 * This algorithm traverses the contour. As it does so, it detects and
		 * fixes various known dangerous span configurations.
		 * 
		 * Traversing the contour: A good way to visualize it is to think of a
		 * robot sitting on the floor facing a known wall. It then does the
		 * following to skirt the wall: 1. If there is a wall in front of it,
		 * turn clockwise in 90 degrees increments until it finds the wall is
		 * gone. 2. Move forward one step. 3. Turn counter-clockwise by 90
		 * degrees. 4. Repeat from step 1 until it finds itself at its original
		 * location facing its original direction.
		 * 
		 * See also: http://www.critterai.org/nmgen_contourgen#robotwalk
		 * 
		 * As the traversal occurs, the number of acute (90 degree) and obtuse
		 * (270 degree) corners are monitored. If a complete contour is detected
		 * and (obtuse corners > acute corners), then the null region is inside
		 * the contour. Otherwise the null region is outside the contour, which
		 * we don't care about.
		 */
		...
		/*
		 * The loop limit exists for the sole reason of preventing an infinite
		 * loop in case of bad input data. It is set to a very high value
		 * because there is no way of definitively determining a safe smaller
		 * value. Setting the value too low can result in rescanning a contour
		...
		while (++loopCount < Integer.MAX_VALUE)
		{
			...
			if (isBorder)
			{
				...
				{
					/*
					 * A border was detected during the last loop as well. Two
					 * detections in a row indicates we passed an acute (inner)
					 * corner.
					 * 
					 * a x x x
					 */
				...
				{
					/*
					 * We have moved at least two spans before detecting a
					 * border. This indicates we passed an obtuse (outer)
					 * corner.
					 * 
					 * a a a x
					 */
			...
			{
				/*
				 * Not a null region border. Move to the neighbor and swing the
				 * search direction back one increment (counterclockwise). By
				 * moving the direction back one increment we guarantee we don't
				 * miss any edges.
				 */
				span = nSpan;
	...
	/**
	 * Detects and fixes span configuration issues in the vicinity of obtuse
	 * (outer) null region corners.
	 * 
	 * @param referenceSpan
	 *            The span in a non-null region that is just past the outer
	 *            corner.
	 * @param borderDirection
	 *            The direciton of the null region border.
	 * @return TRUE if more than one region connects to the null region in the
	 *         vicinity of the corner. (This may or may not be due to a change
	 *         made by this operation.)
	 */
	private boolean processOuterCorner(final OpenHeightSpan referenceSpan,
			final int borderDirection)
	{

		boolean hasMultiRegions = false;

		// Get the previous two spans along the border.
		final OpenHeightSpan backOne = referenceSpan
				.getNeighbor(borderDirection + 3 & 0x3);
				...
				&& backTwo.regionID() == referenceSpan.regionID())
		{
			 ...
			 * result in the region connection being detected as a region-region
			 * portal, when it is not. The region connection is actually
			 * interrupted by the null region.
			 * 
			 * This configuration has been demonstrated to result in two regions
			 * being improperly merged to encompass an internal null region.
			 * 
			 * Example:
			 * 
			 * a a x x x a a a x x a a b b a a a a b b a a a a
			 * 
			 * During contour and connection detection for region b, at no point
			 * will the null region be detected. It will appear as if a clean
			 * a-b portal exists.
			 * 
			 * An investigation into fixing this issue via updates to the
			 * watershed or contour detection algorithms did not turn up a
			 * better way of resolving this issue.
			 */
				...
				&& backTwo.regionID() == referenceSpan.regionID())
		{
			 ...
			 * 
			 * b b x x b a x x <- Short wrap. b a a a
			 * 
			 * In the above case, the short wrap around the corner of the null
			 * region has been demonstrated to cause self-intersecting polygons
			 * during polygon formation.
			 * 
			 * This algorithm detects whether or not one (and only one) of the
			 * axis neighbors of the corner should be re-assigned to a more
			 * appropriate region.
			 * 
			 * In the above example, the following configuration is more
			 * appropriate:
			 * 
			 * b b x x b b x x <- Change to this row. b a a a
			 */
			// Check to see if backTwo should be in a different region.
			int selectedRegion = selectedRegionID(backTwo,
					borderDirection + 1 & 0x3, borderDirection + 2 & 0x3);
			if (selectedRegion == backTwo.regionID())
			{
				...
				selectedRegion = selectedRegionID(referenceSpan,
						borderDirection, borderDirection + 3 & 0x3);
			...
			/*
			 * No dangerous configurations detected. But definitely has a change
			 * in regions at the corner. (We know this because one of the
			 * previous checks looked for a single region for all wrap spans.)
			 */
			hasMultiRegions = true;
	 ...
	 * 
	 * @param referenceSpan
	 *            A span on one side of an null region contour's outer corner.
	 *            It is expected that the all spans that wrap the corner are in
	 *            the same region.
	 * @param borderDirection
	 *            The direction of the null region border.
	 * @param cornerDirection
	 *            The direction of the outer corner from the reference span.
	 * @return The region the span should be a member of. May be the region the
	 *         span is currently a member of.
	 */
	private int selectedRegionID(final OpenHeightSpan referenceSpan,
			final int borderDirection, final int cornerDirection)
	{
		 ...
		 * 
		 * a - Known region. x - Null region. u - Unknown, not checked yet.
		 * 
		 * u u u u a x u a a
		 */
		...
		if (regionID == referenceSpan.regionID()
			...
			/*
			 * The region away from the border is either a null region or the
			 * same region. So we keep the current region.
			 * 
			 * u u u u u u a a x or x a x <-- Potentially bad, but stuck with
			 * it. u a a u a a
			 */
		...
		if (regionID == referenceSpan.regionID() || regionID == NULL_REGION)
			/*
			 * The region opposite from the corner direction is either a null
			 * region or the same region. So we keep the current region.
			 * 
			 * u a u u x u b a x or b a x u a a u a a
			 */
			return referenceSpan.regionID();

		/*
		 * We have checked the early exit special cases. Now a generalized brute
		 * count is performed.
		 * 
		 * Priority is given to the potential region. Here is why: (Highly
		 * unlikely worst case scenario.)
		 * 
		 * c c c c c c b a x -> b b x Select b even though b count == a count. b
		 * a a b a a
		 */
		 ...
		 * b b b b a x b a a
		 * 
		 * The maximum edge case for region A can't exist. It is filtered out
		 * during one of the earlier special cases handlers.
		 * 
		 * Other cases may exist if more regions are involved. Such cases will
		 * tend to favor the current region.
		 */
	 ...
	 * @param span
	 *            The span to check.
	 * @return The direction of the first neighbor in a non-null region, or -1
	 *         if all neighbors are in the null region.
	 */
	 ...
	 * @param span
	 *            The span to check.
	 * @return The direction of the first neighbor that is in the null region,
	 *         or -1 if there are no null region neighbors.
	 */
