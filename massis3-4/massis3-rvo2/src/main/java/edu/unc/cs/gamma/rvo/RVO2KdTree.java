 ...
 */
public class RVO2KdTree implements AgentNeighboursFinder {
	/**
	...
	private RVO2Agent[] agents = null;
	private AgentTreeNode[] agentTree = null;
	private ObstacleTreeNode obstacleTree = null;

	@Override
	public void rebuild(Collection<RVO2Agent> simAgents)
	{
		this.buildAgentTree(simAgents);
	}

	/**
	 * Builds an agent k-D tree.
	 */
	public void buildAgentTree(Collection<RVO2Agent> simAgents)
	{
		if (agents == null || agents.length != simAgents.size())
		{
			agents = new RVO2Agent[simAgents.size()];

			// for (int agentNo = 0; agentNo < agents.length; agentNo++) {
			// agents[agentNo] = simAgents.get(agentNo);
			// }
			{
				int agentNo = 0;
				for (RVO2Agent rvo2Agent : simAgents)
				{
					agents[agentNo] = rvo2Agent;
					agentNo++;
				}
			}
	 ...
	 */
	@Override
	public void computeObstacleNeighbors(RVO2Agent agent, double rangeSq)
	{
	 ...
	 *         otherwise.
	 */
	public boolean queryVisibility(Vector2D q1, Vector2D q2, double radius)
	{
	...
	private void buildAgentTreeRecursive(int begin, int end, int node)
	{
		...
		{
			agentTree[node].maxX = FastMath.max(agentTree[node].maxX,
					agents[i].position.getX());
			agentTree[node].minX = FastMath.min(agentTree[node].minX,
					agents[i].position.getX());
			agentTree[node].maxY = FastMath.max(agentTree[node].maxY,
					agents[i].position.getY());
			agentTree[node].minY = FastMath.min(agentTree[node].minY,
					agents[i].position.getY());
		}
		...
		{
			// No leaf node.
			final boolean isVertical = agentTree[node].maxX
					- agentTree[node].minX > agentTree[node].maxY
							- agentTree[node].minY;
			final double splitValue = 0.5
					* (isVertical ? agentTree[node].maxX + agentTree[node].minX
			...
			{
				while (left < right
						&& (isVertical ? agents[left].position.getX()
								: agents[left].position.getY()) < splitValue)
				{
					left++;
				}

				while (right > left && (isVertical
						? agents[right - 1].position.getX()
						: agents[right - 1].position.getY()) >= splitValue)
				{
					right--;
				}
				...
				if (left < right)
				{
					final RVO2Agent tempAgent = agents[left];
	...
	private static ObstacleTreeNode buildObstacleTreeRecursive(
			List<RVO2Obstacle> obstacles, List<RVO2Obstacle> simObstacles)
	{
		...
		for (int i = 0; i < obstacles.size(); i++)
		{
			...
			final RVO2Obstacle obstacleI1 = obstacles.get(i);
			RVO2Obstacle obstacleI2 = obstacleI1.next;
			...
			for (int j = 0; j < obstacles.size(); j++)
			{
				...
				final RVO2Obstacle obstacleJ1 = obstacles.get(j);
				final RVO2Obstacle obstacleJ2 = obstacleJ1.next;

				final double j1LeftOfI = RVOMath.leftOf(obstacleI1.point,
						obstacleI2.point, obstacleJ1.point);
				final double j2LeftOfI = RVOMath.leftOf(obstacleI1.point,
						obstacleI2.point, obstacleJ2.point);

				if (j1LeftOfI >= -RVOMath.EPSILON
						&& j2LeftOfI >= -RVOMath.EPSILON)
				{
					leftSize++;
				} else if (j1LeftOfI <= RVOMath.EPSILON
						&& j2LeftOfI <= RVOMath.EPSILON)
				{
					rightSize++;
				} else
				...
				final Pair<Integer, Integer> pair1 = new Pair<>(
						FastMath.max(leftSize, rightSize),
						FastMath.min(leftSize, rightSize));
				final Pair<Integer, Integer> pair2 = new Pair<>(
						FastMath.max(minLeft, minRight),
		...
		// Build split node.
		final List<RVO2Obstacle> leftObstacles = new ArrayList<>(minLeft);
		...
		final List<RVO2Obstacle> rightObstacles = new ArrayList<>(minRight);
		...
		final RVO2Obstacle obstacleI1 = obstacles.get(optimalSplit);
		final RVO2Obstacle obstacleI2 = obstacleI1.next;
		...
		for (int j = 0; j < obstacles.size(); j++)
		{
			...
			} else
			{
				...
				final Vector2D splitPoint = obstacleJ1.point
						.add(obstacleJ2.point.subtract(obstacleJ1.point)
								.scalarMultiply(t));

				final RVO2Obstacle newObstacle = new RVO2Obstacle();
				...
				newObstacle.id = simObstacles.size();
		...
		node.left = buildObstacleTreeRecursive(leftObstacles, simObstacles);
		node.right = buildObstacleTreeRecursive(rightObstacles, simObstacles);
	 ...
	 */
	private double queryAgentTreeRecursive(RVO2Agent agent, double rangeSq,
			int node)
	{
		...
		{
			final double distanceSqLeft = sqr(FastMath.max(0.0,
					agentTree[agentTree[node].left].minX
							- agent.position.getX()))
					+ sqr(FastMath.max(0.0,
							agent.position.getX()
									- agentTree[agentTree[node].left].maxX))
					+ sqr(FastMath.max(0.0,
							agentTree[agentTree[node].left].minY
									- agent.position.getY()))
					+ sqr(FastMath.max(0.0, agent.position.getY()
							- agentTree[agentTree[node].left].maxY));
			final double distanceSqRight = sqr(FastMath.max(0.0,
					agentTree[agentTree[node].right].minX
							- agent.position.getX()))
					+ sqr(FastMath.max(0.0,
							agent.position.getX()
									- agentTree[agentTree[node].right].maxX))
					+ sqr(FastMath.max(0.0,
							agentTree[agentTree[node].right].minY
									- agent.position.getY()))
					+ sqr(FastMath.max(0.0, agent.position.getY()
							- agentTree[agentTree[node].right].maxY));

			if (distanceSqLeft < distanceSqRight)
			{
				if (distanceSqLeft < rangeSq)
				{
					rangeSq = queryAgentTreeRecursive(agent, rangeSq,
							agentTree[node].left);

					if (distanceSqRight < rangeSq)
					{
						rangeSq = queryAgentTreeRecursive(agent, rangeSq,
								agentTree[node].right);
					}
				}
			} else
			{
	 ...
	 */
	private static void queryObstacleTreeRecursive(RVO2Agent agent,
			double rangeSq, ObstacleTreeNode node)
	{
		if (node != null)
		{
			final RVO2Obstacle obstacle1 = node.obstacle;
			final RVO2Obstacle obstacle2 = obstacle1.next;

			final double agentLeftOfLine = RVOMath.leftOf(obstacle1.point,
					obstacle2.point, agent.position);

			queryObstacleTreeRecursive(agent, rangeSq,
					agentLeftOfLine >= 0.0 ? node.left : node.right);

			final double distanceSqLine = agentLeftOfLine * agentLeftOfLine
					/ obstacle2.point.distanceSq(obstacle1.point);
	 ...
	 *         otherwise.
	 */
	private static boolean queryVisibilityRecursive(Vector2D q1, Vector2D q2,
			double radius, ObstacleTreeNode node)
	{
		if (node == null)
		{
			return true;
		...
		final double q1LeftOfI = RVOMath.leftOf(obstacle1.point,
				obstacle2.point, q1);
		final double q2LeftOfI = RVOMath.leftOf(obstacle1.point,
				obstacle2.point, q2);
		final double invLengthI = 1.0
				/ obstacle2.point.distanceSq(obstacle1.point);
		...
		if (q1LeftOfI >= 0.0 && q2LeftOfI >= 0.0)
		{
			return queryVisibilityRecursive(q1, q2, radius, node.left)
					&& (q1LeftOfI * q1LeftOfI * invLengthI >= radius * radius
							&& q2LeftOfI * q2LeftOfI * invLengthI >= radius
									* radius
							|| queryVisibilityRecursive(q1, q2, radius,
									node.right));
		}
