 ...
 */
public final class GroupedObjLoader implements AssetLoader {

	private static final Logger logger = Logger
			.getLogger(GroupedObjLoader.class.getName());
	...
	protected final HashMap<String, HashMap<String, ArrayList<Face>>> matFaces = new HashMap<>();
	...
	protected class ObjectGroup {
		...
		public ObjectGroup(final String objectName)
		{
		...
		{
			final Node groupNode = new Node(objectName);
			if (objectName == null)
			{
	...
	protected void findVertexIndex(final Vertex vert)
	{
		final Integer index = vertIndexMap.get(vert);
		if (index != null)
		{
	...
	protected Face[] quadToTriangle(final Face f)
	{
		assert f.verticies.length == 4;

		final Face[] t = new Face[] { new Face(), new Face() };
		t[0].verticies = new Vertex[3];
		t[1].verticies = new Vertex[3];

		final Vertex v0 = f.verticies[0];
		final Vertex v1 = f.verticies[1];
		final Vertex v2 = f.verticies[2];
		final Vertex v3 = f.verticies[3];

		// find the pair of verticies that is closest to each over
		// v0 and v2
		// OR
		// v1 and v3
		final float d1 = v0.v.distanceSquared(v2.v);
		final float d2 = v1.v.distanceSquared(v3.v);
		if (d1 < d2)
		{
	...
	protected void readFace()
	{
		...
		final String[] verticies = line.split("\\s+");
		for (final String vertex : verticies)
		{
			int v = 0;
			int vt = 0;
			int vn = 0;

			final String[] split = vertex.split("/");
			if (split.length == 1)
			{
		...
		{
			final HashMap<String, ArrayList<Face>> groups = matFaces
					.get(currentMatName);
			if (!groups.containsKey(currentObjectName))
			{
				groups.put(currentObjectName, new ArrayList<Face>());
			}
			matFaces.get(currentMatName).get(currentObjectName).add(f);
	...
	protected void loadMtlLib(String name) throws IOException
	{
		...
		final AssetKey<?> mtlKey = new AssetKey<>(key.getFolder() + name);
		try
		{
			matList = (MaterialList) assetManager.loadAsset(mtlKey);
		} catch (final AssetNotFoundException ex)
		{
			logger.log(Level.WARNING, "Cannot locate {0} for model {1}",
					new Object[] { name, key });
		}
		...
		if (matList != null)
		{
			...
			for (final String matName : matList.keySet())
			{
				matFaces.put(matName, new HashMap<>());
			}
		}
	}
	...
	protected boolean readLine() throws IOException
	{
		...
		} else if (cmd.equals("usemtl"))
		{
			...
			// throw new IOException("Cannot locate material " + currentMatName
			// + " in MTL file!");

		} else if (cmd.equals("mtllib"))
		{
			// specify MTL lib to use for this OBJ file
			final String mtllib = scan.nextLine().trim();
			loadMtlLib(mtllib);
		} else if (cmd.equals("g"))
		{
			currentObjectName = scan.next();
	...
	protected Geometry createGeometry(final ArrayList<Face> faceList,
			final String matName) throws IOException
	{
		if (faceList.isEmpty())
			throw new IOException("No geometry data to generate mesh");

		// Create mesh from the faces
		final Mesh mesh = constructMesh(faceList);

		final Geometry geom = new Geometry(objName + "-geom-" + geomIndex++,
				mesh);
	...
	protected Mesh constructMesh(final ArrayList<Face> faceList)
	{
		...
		final ArrayList<Face> newFaces = new ArrayList<Face>(faceList.size());
		for (int i = 0; i < faceList.size(); i++)
		{
			final Face f = faceList.get(i);

			for (final Vertex v : f.verticies)
			{
				findVertexIndex(v);
		...
		final FloatBuffer posBuf = BufferUtils
		...
		if (vertIndexMap.size() >= 65536)
		{
			...
			final IntBuffer ib = BufferUtils
					.createIntBuffer(newFaces.size() * 3);
			m.setBuffer(VertexBuffer.Type.Index, 3, ib);
			indexBuf = new IndexIntBuffer(ib);
		} else
		{
			final ShortBuffer sb = BufferUtils
					.createShortBuffer(newFaces.size() * 3);
			m.setBuffer(VertexBuffer.Type.Index, 3, sb);
			indexBuf = new IndexShortBuffer(sb);
		}

		final int numFaces = newFaces.size();
		for (int i = 0; i < numFaces; i++)
		{
			final Face f = newFaces.get(i);
			...
			final int index = i * 3; // current face * 3 = current index
	...
	@Override
	public Object load(final AssetInfo info) throws IOException
	{
		reset();
		...
		final String folderName = key.getFolder();
		...
		{
			// for (Entry<String, ArrayList<Face>> entry : matFaces.entrySet()){
			for (final Entry<String, HashMap<String, ArrayList<Face>>> entry : matFaces
					.entrySet())
			{
				final HashMap<String, ArrayList<Face>> materialGroups = entry
						.getValue();
				for (final Entry<String, ArrayList<Face>> geomEntry : materialGroups
						.entrySet())
				{
					final ArrayList<Face> materialFaces = geomEntry.getValue();
					if (materialFaces.size() > 0)
					{
						final Geometry geom = createGeometry(materialFaces,
								entry.getKey());
						geom.setName(geomEntry.getKey());
						objNode.attachChild(geom);
					}
				}
		...
		{
			// generate final geometry
			final Geometry geom = createGeometry(faces, null);
			objNode.attachChild(geom);
		}
