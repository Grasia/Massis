...
public class ArrayQuadTree<E> {
	...
	private CoordinateRetriever<E> coordRetriever;

	@SuppressWarnings("unchecked")
	public ArrayQuadTree(
			final int noLevels,
			int minX,
			int maxX,
			int minY,
			int maxY,
			CoordinateRetriever<E> coordRetriever)
	{
		this.coordRetriever = coordRetriever;
		this.minX = minX;
		this.maxX = maxX;
		this.minY = minY;
		this.maxY = maxY;
	...
	public void insert(E e)
	{
		...
		final int x = (int) (coordRetriever.getX(e) * this.inv_cellSizeX - this.inv_cellSizeX_minX);
		final int y = (int) (coordRetriever.getZ(e) * this.inv_cellSizeY - this.inv_cellSizeY_minY);
			...
			double ry1, ArrayQuadTreeCallback<E> callback)
	{
		...
		if (level == this.noLevels - 1)
		{
			...
			for (final E elem : this.childNodes[x][y])
			{
				...
				if (contains(rx0, ry0, rx1, ry1, this.coordRetriever.getX(elem),
						this.coordRetriever.getZ(elem)))
				{
					callback.query(elem);
				}
			}
	...
	public static interface CoordinateRetriever<E> {
		public float getX(E e);

		public float getZ(E e);
	}

	public static interface ArrayQuadTreeCallback<E> {
