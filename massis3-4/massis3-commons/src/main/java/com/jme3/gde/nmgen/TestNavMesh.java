...
public class TestNavMesh extends SimpleApplication {
	...
	{
		final TestNavMesh test = new TestNavMesh();
		final AppSettings settings = new AppSettings(false);// vrAppState.getSettings();
	...
	public void simpleInitApp()
	{
		...
		final Node world = (Node) assetManager.loadModel("Models/navtest.j3o");

		final NavMeshController controller = new NavMeshController(world);

		final Mesh navMesh = controller.generateNavMesh(0.15f, 0.5f, 1.5f,
				0.25f, 48f, true, 0.1f, 2f, true, 3, 1, 0f, 0.25f, 6, 100,
				0.1f);

		final Geometry navGeom = new Geometry("NavMesh", navMesh);
		final Material m = new Material(assetManager,
