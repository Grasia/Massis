...
class Heap {
	...
	private final MinHeap<Node> nodes = new MinHeap<>();
	...
	void initialize(final int sessionID, final Vector3f goal)
	{
		this.goal = goal;
	...
	void addCell(final Cell pCell)
	{
		final Node newNode = new Node(pCell, pCell.getTotalCost());
		nodes.add(newNode);
	}
	 ...
	 */
	void adjustCell(final Cell pCell)
	{
		final Node n = findNodeIterator(pCell);
	 ...
	 * unless nessesary.
	 */
	Node findNodeIterator(final ICell pCell)
	{
