...
public class TestSteering extends SimpleApplication {
	...
	{
		final TestSteering app = new TestSteering();
		app.start();
	}
	...
	public void simpleInitApp()
	{
		...
		final Quaternion YAW180 = new Quaternion().fromAngleAxis(FastMath.PI,
				new Vector3f(0, 1, 0));
		...
		{
			final Vehicle obstacle = new Vehicle(ColorRGBA.Yellow);
			obstacle.setLocalTranslation((float) Math.random() * 10f, 0,
		...
		for (int i = 0; i < amount; i++)
		{
			final Vehicle neighbour = new Vehicle(ColorRGBA.Blue);
			neighbour.setLocalTranslation((float) Math.random() * 5f, 0,
	 ...
	 */
	private void fillObstacals(final List<Obstacle> obstacles)
	{
		for (final Spatial s : obstacleNode.getChildren())
		{
			if (s instanceof Vehicle)
			{
				final Vehicle v = (Vehicle) s;
	 ...
	 */
	private void fillNeighbours(final Vehicle source,
			final List<Obstacle> neighbours, final float radius)
	{
		final float r2 = radius * radius;
		for (final Spatial s : friendNode.getChildren())
		{
			if (s instanceof Vehicle && !s.equals(source))
			{
				final Vehicle v = (Vehicle) s;
				final float d = source.getWorldTranslation()
						.subtract(v.getWorldTranslation()).lengthSquared();
	...
	protected class Vehicle extends Node {
		...
		float maxTurnForce = 2f; // max steering force per second (perpendicular
									// to velocity)
									// if speed is 1 and turn force is 1, then
									// it will turn 45 degrees in a second
		...
		protected Vehicle(final ColorRGBA color)
		{

			final Box b = new Box(Vector3f.ZERO, 0.1f, 0.1f, 0.2f);
		 ...
		 */
		protected void updateVelocity(final Vector3f steeringInfluence,
				final float scale)
		{
			// showVelocity(velocity.length(), true);
			// showCollisionRange(speed/maxTurnForce, true);
			// if (true) return;
			final Vector3f steeringForce = truncate(steeringInfluence,
					maxTurnForce * scale);
			final Vector3f acceleration = steeringForce.divide(mass);
			final Vector3f vel = truncate(velocity.add(acceleration), maxSpeed);
			velocity = vel;

			setLocalTranslation(
					getLocalTranslation().add(velocity.mult(scale)));

			// rotate to face
			final Quaternion rotTo = getLocalRotation().clone();
		 ...
		 */
		private Vector3f truncate(final Vector3f source, final float limit)
		{
		...
		/**
		 * Gets the predicted position for this 'frame', taking into account
		 * current position and velocity.
		 * 
		 * @param tpf
		 *            time per fram
		 */
		protected Vector3f getFuturePosition(final float tpf)
		{
			return getWorldTranslation().add(velocity);
		}

		protected void showVelocity(final float length, final boolean show)
		{
			if (velocityLine == null)
			{
				// create it if it doesn't exist
				// Vector3f end = this.velocity.normalize().mult(length);
				final Line line = new Line(Vector3f.ZERO,
						new Vector3f(0, 0, length));
				final Geometry geom = new Geometry("cylinder", line);
				final Material mat = new Material(assetManager,
						"Common/MatDefs/Misc/Unshaded.j3md");
			...
			final Line line = (Line) velocityLine.getMesh();
		...
		private void showCollisionRange(final float length, final boolean show)
		{
			...
			// modify its direction and length
			final Line line = (Line) collisionLine.getMesh();
			line.updatePoints(Vector3f.ZERO, new Vector3f(0, 0, length));
		...
		protected void setCollideWarning(final boolean warn)
		{
			if (collisionLine != null)
			{
				if (warn)
				{
					collisionLine.getMaterial().setColor("Color",
							ColorRGBA.Red);
				} else
				{
					collisionLine.getMaterial().setColor("Color",
		...
		{
			return new SimpleObstacle(getWorldTranslation(), collisionRadius,
					velocity);
		}
	}
	...
	private class SeekControl extends SimpleControl {
		...
		{
			final Vehicle vehicle = (Vehicle) getSpatial();
			if (vehicle == null)
				return;

			// calculate the steering force from the Seek routine
			final Vector3f steering = seek.calculateForce(
					vehicle.getWorldTranslation(),
	...
	private class FleeControl extends SimpleControl {
		...
		protected void controlUpdate(final float tpf)
		{
			...
			final Vector3f steering = flee.calculateForce(
					vehicle.getWorldTranslation(),
					vehicle.velocity, vehicle.speed,
					target.getWorldTranslation());
	...
	private class PersuitControl extends SimpleControl {
		...
		protected void controlUpdate(final float tpf)
		{
			...
			final Vector3f steering = persuit.calculateForce(
					vehicle.getWorldTranslation(),
					vehicle.velocity,
					vehicle.speed,
					target.speed,
	...
	private class EvadeControl extends SimpleControl {
		...
		protected void controlUpdate(final float tpf)
		{
			...
			// calculate the steering force from the Persuit routine
			final Vector3f steering = evade.calculateForce(
					vehicle.getWorldTranslation(),
					vehicle.velocity,
					vehicle.speed,
					target.speed,
	...
	private class PersuitAndAvoidControl extends SimpleControl {
		...
		protected void controlUpdate(final float tpf)
		{
			...
			final Vector3f avoidance = avoid.calculateForce(
					vehicle.getWorldTranslation(),
					vehicle.velocity,
					vehicle.speed,
	...
	private class PersuitSeparationControl extends SimpleControl {
		...
		protected void controlUpdate(final float tpf)
		{
			...
			final Vector3f separate = separation.calculateForce(
					vehicle.getWorldTranslation(),
					vehicle.velocity,
					neighbours);
	...
	private class PersuitAvoidSeparationControl extends SimpleControl {
		...
		protected void controlUpdate(final float tpf)
		{
			...
			neighbours.clear(); // re-calculate every time
			fillNeighbours(vehicle, neighbours, 0.4f);
	...
	/**
	 * Just implements some of the required methods to make this test case
	 * easier.
	 */
