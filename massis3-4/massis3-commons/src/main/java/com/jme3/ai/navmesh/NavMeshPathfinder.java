...
import com.jme3.ai.navmesh.ICell.PathResult;
...
public class NavMeshPathfinder {
	...
	private final Vector3f currentPos3d = new Vector3f();
	...
	public NavMeshPathfinder(final NavMesh navMesh)
	{
	...
	public void setEntityRadius(final float entityRadius)
	{
		this.entityRadius = entityRadius;
	}

	/**
	 * Warp into the scene on the nav mesh, finding the nearest cell. The
	 * currentCell and position3d are updated and this new position is returned.
	 * This updates the state of the path finder!
	 * 
	 * @return the new position in the nearest cell
	 */
	public Vector3f warp(final Vector3f newPos)
	{
		final Vector3f newPos2d = new Vector3f(newPos.x, 0, newPos.z);
		currentCell = navMesh.findClosestCell(newPos2d);
		currentPos3d.set(currentCell.snapPoint(newPos2d));
	...
	/**
	 * Get the nearest cell to the supplied position and place the returned
	 * position in that cell.
	 * 
	 * @param position
	 *            to place in the nearest cell
	 * @return the position in the cell
	 */
	public Vector3f warpInside(final Vector3f position)
	{
		final Vector3f newPos2d = new Vector3f(position.x, 0, position.z);
		final Cell cell = navMesh.findClosestCell(newPos2d);
		position.set(cell.snapPoint(newPos2d));
		return position;
	}
	 ...
	 */
	/*
	 * public boolean isInsideNavMesh(Vector3f position) { Cell cell =
	 * navMesh.findClosestCell(position); return cell != null; }
	 */
	/**
	 * Generate a new Path from the currentPos3d to the supplied goal position.
	 * setPosition() must be called first for this to work. If the point is not
	 * in the mesh, false is returned. You should use warp() in that case to
	 * place the point in the mesh.
	 * 
	 * @return fail if no path found or start location outside of a cell
	 */
	public boolean computePath(final Vector3f goal)
	{
	 ...
	 * @return fail if no path found or start location outside of a cell
	 */
	public boolean computePath(final Vector3f goal, final DebugInfo debugInfo)
	{
		// get the cell that this point is in
		final Vector3f newPos2d = new Vector3f(currentPos3d.x, 0,
				currentPos3d.z);
		currentCell = navMesh.findClosestCell(newPos2d);
		if (currentCell == null)
		{
		...
		final Vector3f goalPos2d = new Vector3f(goalPos.getX(), 0,
				goalPos.getY());
		goalCell = navMesh.findClosestCell(goalPos2d);
		final boolean result = buildNavigationPath(path, currentCell,
				currentPos3d, goalCell, goalPos3d, entityRadius, debugInfo);
		if (!result)
		{
			goalPos = null;
			goalCell = null;
	...
	{
		final Vector3f waypt = nextWaypoint.getPosition();
	...
	public Vector3f onMove(final Vector3f moveVec)
	{
		...
		final Vector3f newPos2d = new Vector3f(currentPos3d);
		newPos2d.addLocal(moveVec);
		newPos2d.setY(0);

		final Vector3f currentPos2d = new Vector3f(currentPos3d);
		currentPos2d.setY(0);

		// Cell nextCell = navMesh.resolveMotionOnMesh(currentPos2d,
		// currentCell, newPos2d, newPos2d);
	...
	public void goToNextWaypoint(final DebugInfo debugInfo)
	{
		final int from = getPath().getWaypoints().indexOf(nextWaypoint);
		...
		final int to = getPath().getWaypoints().indexOf(nextWaypoint);
	 ...
	 */
	private boolean buildNavigationPath(final Path navPath,
			final Cell startCell, final Vector3f startPos,
			final Cell endCell, final Vector3f endPos,
			final float entityRadius, final DebugInfo debugInfo)
	{
		...
		while (heap.isNotEmpty() && !foundPath)
		{
			...
			// Heap
			final Node currentNode = heap.getTop();
		...
		final Vector2f intersectionPoint = new Vector2f();
		...
		while (currentCell != null && currentCell != endCell)
		{
			...
			// wall's center)
			final int linkWall = currentCell.getArrivalWall();
			Vector3f newWayPoint = currentCell.getWallMidpoint(linkWall)
					.clone();

			final Line2D wall = currentCell.getWall(linkWall);
			final float length = wall.length();
			final float distBlend = entityRadius / length;

			final Line2D lineToGoal = new Line2D(
					new Vector2f(lastWayPoint.x, lastWayPoint.z),
					new Vector2f(endPos.x, endPos.z));
			final LineIntersect result = lineToGoal.intersect(wall,
					intersectionPoint);
			switch (result)
			{
			case SegmentsIntersect:
				final float d1 = wall.getPointA().distance(intersectionPoint);
				final float d2 = wall.getPointB().distance(intersectionPoint);
				if (d1 > entityRadius && d2 > entityRadius)
				{
					// we can fit through the wall if we go
					// directly to the goal.
					newWayPoint = new Vector3f(intersectionPoint.x, 0,
							intersectionPoint.y);
				} else
				{
					// cannot fit directly.
					...
					if (d1 < d2)
					{
						intersectionPoint.interpolateLocal(wall.getPointA(),
								wall.getPointB(), distBlend);
						newWayPoint = new Vector3f(intersectionPoint.x, 0,
								intersectionPoint.y);
					} else
					{
						intersectionPoint.interpolateLocal(wall.getPointB(),
								wall.getPointA(), distBlend);
						newWayPoint = new Vector3f(intersectionPoint.x, 0,
								intersectionPoint.y);
					}
				}
			...
			case ABisectsB:
			case BBisectsA:
				final Vector2f lastPt2d = new Vector2f(lastWayPoint.x,
						lastWayPoint.z);
				final Vector2f endPos2d = new Vector2f(endPos.x, endPos.z);

				final Vector2f normalEnd = endPos2d.subtract(lastPt2d)
						.normalizeLocal();
				final Vector2f normalA = wall.getPointA().subtract(lastPt2d)
						.normalizeLocal();
				final Vector2f normalB = wall.getPointB().subtract(lastPt2d)
						.normalizeLocal();
		...
		final List<Waypoint> newPath = new ArrayList<Waypoint>();
		...
		navPath.initialize(navMesh, startPos, startCell, endPos, endCell);
		for (final Waypoint newWayPoint : newPath)
		{
			navPath.addWaypoint(newWayPoint.getPosition(),
					newWayPoint.getCell());
		}
		navPath.finishPath();
	 ...
	 */
	@SuppressWarnings("unused")
	private ICell resolveMotionOnMesh(final Vector3f startPos,
			final Cell startCell, final Vector3f endPos,
			final Vector3f modifiedEndPos)
	{
		int i = 0;
		// create a 2D motion path from our Start and End positions, tossing out
		// their Y values to project them
		// down to the XZ plane.
		final Line2D motionLine = new Line2D(
		...
		do
		{
			...
			if (result.result == PathResult.ExitingCell)
			{
				...
				} else
				{
					...
					final Vector2f Direction = motionLine.getPointB().subtract(
							motionLine.getPointA()).mult(0.9f);
					// Direction.mult(0.9f);
					motionLine.setPointB(motionLine.getPointA().add(
							Direction));
				}
			} else if (result.result == ICell.PathResult.NoRelationship)
			{
				...
				final Vector2f NewOrigin = motionLine.getPointA();
				// NewOrigin.x -= 0.01f;
			...
			// friction
			//
		while (result.result != ICell.PathResult.EndingCell
	 ...
	 * @return
	 */
	public ICell DEBUG__getCurrentCell()
	{
		return currentCell;
	}
	 ...
	 * @return
	 */
	public ICell DEBUG__getGoalCell()
	{
		return goalCell;
	}
