 ...
 * 
 */
public class Cell implements Savable, ICell {
	...
	public Vector3f snapPoint(final Vector3f point)
	{
		...
		{
			this.forcePointToCellColumn(point);
		}

		this.computeHeightOnCell(point);
		return point;
	}
	 ...
	 */
	private final Plane cellPlane = new Plane();

	/**
	 * pointers to the verticies of this triangle held in the NavigationMesh's
	 * vertex pool
	 */
	private final Vector3f[] verticies = new Vector3f[3];
	 ...
	 */
	private final Vector3f[] wallMidpoints = new Vector3f[3];

	/**
	 * the distances between each wall midpoint of sides (0-1, 1-2, 2-0)
	 */
	private final float[] wallDistances = new float[3];
	private int index;

	@Override
	public void initialize(final Vector3f pointA, final Vector3f pointB,
	...
	void computeCellData()
	{
		...
		final Vector2f point1 = new Vector2f(verticies[VERT_A].x,
				verticies[VERT_A].z);
		final Vector2f point2 = new Vector2f(verticies[VERT_B].x,
				verticies[VERT_B].z);
		final Vector2f point3 = new Vector2f(verticies[VERT_C].x,
				verticies[VERT_C].z);
		...
		center.x = (verticies[VERT_A].x + verticies[VERT_B].x
				+ verticies[VERT_C].x) / 3;
		center.y = (verticies[VERT_A].y + verticies[VERT_B].y
				+ verticies[VERT_C].y) / 3;
		center.z = (verticies[VERT_A].z + verticies[VERT_B].z
				+ verticies[VERT_C].z) / 3;
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.jme3.ai.navmesh.ICell#getTriangle()
	 */
	@Override
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.jme3.ai.navmesh.ICell#isLeft(com.jme3.math.Vector3f,
	 ...
	 * @return
	 */
	boolean requestLink(final Vector3f pointA, final Vector3f pointB,
			final Cell caller,
			final float epsilon)
	{
	 ...
	 * @param Caller
	 */
	private void setLink(final int Side, final Cell Caller)
	{
		links[Side] = Caller;
	}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.jme3.ai.navmesh.ICell#getHeightOnCell(com.jme3.math.Vector3f)
	 */
	@Override
	public float getHeightOnCell(final Vector3f point)
	{
	 ...
	 * 
	 * @see
	 * com.jme3.ai.navmesh.ICell#computeHeightOnCell(com.jme3.math.Vector3f)
	 */
	@Override
	public void computeHeightOnCell(final Vector3f point)
	{
	...
	@Override
	public boolean contains(final float point_x, final float point_y)
	{
		...
		for (int i = 0; i < 3; i++)
		{
			final Line2D.PointSide SideResult = sides[i].getSide(
					point_x, point_y, 1.0e-6f);
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.jme3.ai.navmesh.ICell#contains(com.jme3.math.Vector3f)
	 */
	@Override
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.jme3.ai.navmesh.ICell#getVertex(int)
	 */
	@Override
	public Vector3f getVertex(final int Vert)
	{
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.jme3.ai.navmesh.ICell#getCenter()
	 */
	@Override
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.jme3.ai.navmesh.ICell#getLink(int)
	 */
	@Override
	public Cell getLink(final int side)
	{
		return links[side];
	}

	@Override
	public Line2D getWall(final int side)
	{
		return sides[side];
	}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.jme3.ai.navmesh.ICell#getWallLength(int)
	 */
	@Override
	public float getWallLength(final int side)
	{
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.jme3.ai.navmesh.ICell#getWallMidpoint(int)
	 */
	@Override
	public Vector3f getWallMidpoint(final int side)
	{
	...
	public ClassifyResult classifyPathToCell(final Line2D motionPath)
	{
		...
		for (int i = 0; i < 3; ++i)
		{
			...
			final Line2D.PointSide end = sides[i].getSide(
			...
			{// (end != Line2D.PointSide.Right) {
				...
				if (sides[i].getSide(motionPath.getPointA(),
						0.0f) != Line2D.PointSide.Left)
				{
					...
					final Line2D.LineIntersect IntersectResult = motionPath
							.intersect(sides[i], result.intersection);
	 ...
	 */
	void projectPathOnCellWall(final int sideNumber, final Line2D motionPath)
	{
		...
		final float DotResult = MotionVector.dot(WallNormal);
	 ...
	 * @return
	 */
	boolean forcePointToWallInterior(final int sideNumber, final Vector2f point)
	{
	 ...
	 * @return
	 */
	boolean forcePointToWallInterior(final int sideNumber, final Vector3f point)
	{
		final Vector2f TestPoint2D = new Vector2f(point.x, point.z);
		final boolean PointAltered = forcePointToWallInterior(sideNumber,
				TestPoint2D);
	 ...
	 */
	@Override
	public boolean forcePointToCellColumn(final Vector2f point)
	{
		// create a motion path from the center of the cell to our point
		final Line2D TestPath = new Line2D(new Vector2f(center.x,
				center.z), point);

		final ClassifyResult result = classifyPathToCell(TestPath);
	 ...
	 */
	@Override
	public boolean forcePointToCellColumn(final Vector3f point)
	{
		final Vector2f TestPoint2D = new Vector2f(point.x, point.z);
		final boolean PointAltered = forcePointToCellColumn(TestPoint2D);
	 ...
	 * @return
	 */
	boolean processCell(final Heap heap)
	{
	 ...
	 * @return
	 */
	boolean queryForPath(final Heap heap, final ICell caller,
			final float arrivalCost)
	{
		if (sessionID != heap.getSessionID())
		{
	 ...
	 * @param goal
	 */
	void computeHeuristic(final Vector3f goal)
	{
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.jme3.ai.navmesh.ICell#toString()
	 */
	@Override
	public String toString()
	{
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.jme3.ai.navmesh.ICell#getNormal()
	 */
	@Override
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.jme3.ai.navmesh.ICell#getRandomPoint()
	 */
	@Override
	public Vector3f getRandomPoint()
	{
		final Random rand = new Random();
		final Vector2f ret = this.sides[0].getPointA()
				.add(this.sides[0].getDirection().mult(rand.nextFloat()).add(
						this.sides[1].getDirection().mult(rand.nextFloat())));
		forcePointToCellColumn(ret);
		final Vector3f vec = new Vector3f(ret.x, 0, ret.y);
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.jme3.ai.navmesh.ICell#write(com.jme3.export.JmeExporter)
	 */
	@Override
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.jme3.ai.navmesh.ICell#read(com.jme3.export.JmeImporter)
	 */
	@Override
	public void read(final JmeImporter e) throws IOException
	{
		...
		final Savable[] verts = capsule.readSavableArray("verticies", null);
		for (int i = 0; i < verts.length; i++)
		{
			verticies[i] = (Vector3f) verts[i];
		}

		final Savable[] savLinks = capsule.readSavableArray("links", null);
		...
		// wallMidpoints = (Vector3f[]) capsule.readSavableArray("midpoints",
		// new Vector3f[3]);
		// wallDistances = capsule.readFloatArray("distances", new float[3]);

	}

	public void checkAndLink(final Cell other, final float epsilon)
	{
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.jme3.ai.navmesh.ICell#getDebugMesh()
	 */
	@Override
	public Mesh getDebugMesh()
	{
		final Mesh m = new Mesh();// (verticies[0], verticies[1], verticies[2]);
		m.setBuffer(Type.Position, 3,
				new float[] { verticies[0].x, verticies[0].y, verticies[0].z,
						verticies[1].x, verticies[1].y, verticies[1].z,
						verticies[2].x, verticies[2].y, verticies[2].z });

		// the tex coords are wrong, but we render wire/unshaded so it doesn't
		// matter
	...
	@Override
	public void setLink(final int side, final ICell link)
	{
		this.links[side] = null;
	}
	...
	public boolean intersectsCircle(final Vector3f point, final float radius)
	{
		...
		{
			if (LinesUtils
					.closestPointOnLineSegment(this.getVertex(i),
							this.getVertex(i % this.getNumLinks()), point)
