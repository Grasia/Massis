...
public abstract class Heap<T> extends Vector<T> {
	 ...
	 */
	public Heap(final Collection<T> c)
	{
		super();
		addAll(c);
	}
	...
	public boolean add(final T o)
	{
		...
		for (int node = size() - 1; node > 0;)
		{
			...
			final int cmp;
			final int parent = (node - 1) / 2;

			if (cmp(node, parent) < 0)
			{
				// swap them and reheapify
				final T tmp = get(node);
				set(node, get(parent));
				set(parent, tmp);
			}
	 ...
	 */
	@SuppressWarnings("unchecked")
	protected int cmp(final int node1, final int node2)
	{
		int c = 0;
		if (comp != null)
		{
	 ...
	 * property
	 */
	private void heapify(final int node, final int size)
	{
		if (node > size)
		{
			return;
		...
		final int left = (node + 1) * 2 - 1;
		final int right = (node + 1) * 2;
